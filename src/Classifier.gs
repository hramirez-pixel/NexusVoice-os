...
function classifyIncomingMessage(text, apiKey) {
  ...
  - Para COMPLETAR_TAREA: "titulos_tarea" es un ARREGLO con el texto aproximado de cada tarea completada (obligatorio, al menos un elemento — sin esto no se puede encontrar cuál tarea es; casi siempre trae 1 elemento, pero puede traer varios si el mensaje menciona más de una). "nota_tarea" es cualquier nota/comentario que el usuario deje sobre cómo, cuándo o con qué resultado se hizo (ej. "ya hice lo de enviar reportes, se mandó por correo a las 3" → titulos_tarea: ["enviar reportes"], nota_tarea: "se mandó por correo a las 3"), o null si no dejó ninguna — se aplica igual a todas las tareas del arreglo. También acepta "destino_agenda"/"persona_agenda" igual que arriba, por si la(s) tarea(s) son de otra persona registrada.
