...
function classifyIncomingMessage(text, apiKey) {
  ...
  - COMPLETAR_TAREA: avisa que una TAREA/pendiente YA CREADA se terminó/hizo/completó, por su título aproximado (no por ID — las tareas no tienen ID corto). Reconócelo por el SIGNIFICADO ("ya hice...", "ya terminé...", "márcalo/marca como hecho/completado/completada/lista...", "termina la tarea de..."), no por frase exacta. Ejemplo: "Marca como completada la tarea pendiente de enviar reportes" → intent: COMPLETAR_TAREA, titulo_tarea: "enviar reportes" (la palabra "pendiente" aquí es solo describiendo QUÉ tarea es, NO la disparas a CONSULTAR_PENDIENTES — el verbo "marca/completa" manda, no la palabra "pendiente"). Si en el mismo mensaje deja una nota sobre cómo/cuándo se hizo, extráela aparte — NUNCA confundas esto con crear una tarea nueva (modo NOTA).
